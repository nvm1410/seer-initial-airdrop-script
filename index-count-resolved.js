...
import { getRandomNextDayTimestamp, getRandomTimestamps, parseToCsv, isTwoStringsEqual, convertToFinalCSV, mergeTokenBalances, findInvalidDay } from './src/utils.js';
...
import bunniTokens from './data/bunniTokens-1.json' with {type: 'json'}

import { getBunniLpTokensTransferEvents, getBunniPositionHoldersAtTimestamp } from './src/getLpTokens.js';

import adjust from './data/adjust-100.json' with {type: 'json'}
import original from './data/original-100.json' with {type: 'json'}
...
async function getAll(chainId) {
    ...
    // console.log(final)
    // const transfers = await getBunniLpTokensTransferEvents(bunniTokens)
    // fs.writeFileSync('./data/positionSnapshots-1.json', JSON.stringify(transfers, null, 4))
    // getSeedOutcomeTokensValueSnapshots(chainId)
    // getPositionSnapshotsByTokenPairs(100, tokens)
...
async function distributeAirdrop() {
    ...
    const timestampToUsers100 = adjust
    const finalData = []
    for (const timestamp of timestamps) {
    ...
    fs.writeFileSync('seed-adjust.json', JSON.stringify(finalData, null, 4))
}
